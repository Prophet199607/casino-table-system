...
import TransferRefill from "./TransferRefill";
...
export const MenuModal: React.FC<MenuProps> = ({ onAction, onClose }) => {
  ...
  const handleButtonClick = (label: MenuAction) => {
    if (label === "Table Result") {
      setShowTableResults(true);
    }
    else if (label === "Transfer & Refill Accept") {
      setShowTransferRefill(true);
    }
    else if (label === "Chip Details") {
